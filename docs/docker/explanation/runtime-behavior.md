---
title: Поведение среды исполнения (Runtime)
type: explanation
tags: [docker, runtime, signals, logging, exit-codes, oom-killer, restart-policy, pid1]
sources:
...
> **TL;DR:** Контейнер = процесс с PID 1. `docker stop` → SIGTERM → 10 сек → SIGKILL.
> Exit code 137 = OOM Kill. Без ротации логов Docker заполнит весь диск.
