...
> **TL;DR:** Контейнеры общаются через виртуальные сети (bridge). DNS по именам работает
> только в user-defined networks. Docker манипулирует iptables напрямую — UFW может не помочь.
