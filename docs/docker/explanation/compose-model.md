---
title: Модель композиции (Docker Compose)
type: explanation
tags: [docker, compose, iac, declarative, services, healthchecks, profiles]
sources:
...
> **TL;DR:** Docker Compose — декларативное описание многоконтейнерного приложения.
> Один YAML-файл заменяет десятки `docker run` команд. Service ≠ Container.
