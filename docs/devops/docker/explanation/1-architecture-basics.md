...
type: explanation
tags: [docker, architecture, daemon, containerd, runc, namespaces, cgroups, rottless]
---
...
*   **UNIX Socket (`/var/run/docker.sock`)**: Основной канал связи. Файл принадлежит пользотелю `root` и группе `docker`.
